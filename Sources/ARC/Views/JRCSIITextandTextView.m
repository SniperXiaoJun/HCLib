...
@interface JRCSIITextandTextView ()<UITextFieldDelegate>{
...
static JRCSIITextandTextView *csiiTextandTextView = nil;

@implementation JRCSIITextandTextView
...
+(JRCSIITextandTextView *)shareInstance{
    ...
    dispatch_once(&onceToken, ^{
        csiiTextandTextView = [[JRCSIITextandTextView alloc] initWithFrame:CGRectMake((ScreenWidth - 645 * scaleW)/2, (ScreenHeight - 745 *scaleH)/2, 645 * scaleW, 845 * scaleH)];
        [((UIWindow*)[[[UIApplication sharedApplication] windows] objectAtIndex:0]).rootViewController.view addSubview:csiiTextandTextView];
    });
    
    return csiiTextandTextView;
}
...
- (void)test1 {
    NSString*filePath = [[NSBundle mainBundle]pathForResource:@"TextAndText"ofType:@"json"];
...
-(void)show:(JRCSIITextandTextViewBlock)JRCSIITextandTextViewBlock{
    ...
    self.JRCSIITextandTextViewBlock = JRCSIITextandTextViewBlock;
    retDicView = [[UIView alloc] initWithFrame:CGRectMake(15, 35+50*scaleH, csiiTextandTextView.frame.size.width-15, 350*scaleH)];
    [self addSubview:retDicView];
    
    for (int i=0; i<type; i++) {
        leftLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 550*scalePH/type*i, csiiTextandTextView.frame.size.width/type, 350*scalePH/type)];
        leftLabel.text = [[uiData objectAtIndex:i] valueForKey:@"LeftText"];
        [retDicView addSubview:leftLabel];
        
        rightLabel = [[UILabel alloc] initWithFrame:CGRectMake(5+csiiTextandTextView.frame.size.width/type , 550*scalePH/type*i, csiiTextandTextView.frame.size.width/2, 350*scalePH/type)];
//        rightLabel.backgroundColor = [UIColor orangeColor];
    ...
    popAnimation.timingFunctions = @[[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut],
                                     ...
                                     [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut]];
    [csiiTextandTextView.layer addAnimation:popAnimation forKey:nil];
    csiiTextandTextView.hidden = NO;
}
...
-(void)creatPasswordUI{
    ...
    pwdTF = [[PowerEnterUITextField alloc] initWithFrame:CGRectMake(0, 0, passWordimg.frame.size.width, 150*scalePH)];
    ...
    CGFloat width = (image.frame.size.width)/ kDotCount;
    
    dotArray = [[NSMutableArray alloc] init];
    //生成中间的点
    for (int i = 0; i < kDotCount; i++) {
        dotView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetMinX(pwdTF.frame) + (width - kDotCount) / 2 + i * width, CGRectGetMinY(pwdTF.frame) + (image.frame.size.height - kDotSize.height) / 2, kDotSize.width, kDotSize.height)];
...
- (void)cancelLoginTaped{
    ...
    csiiTextandTextView.hidden = YES;
    pwdTF.text = nil;
...
-(void)submitConfirm{
    ...
    //TODO: 这里执行block
    if (self.JRCSIITextandTextViewBlock) {
        self.JRCSIITextandTextViewBlock();
    }
