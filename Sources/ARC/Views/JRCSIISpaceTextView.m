...
@interface JRCSIISpaceTextView ()<UITextFieldDelegate>{
...
static JRCSIISpaceTextView *csiiSpaceTextView = nil;

@implementation JRCSIISpaceTextView
...
+(JRCSIISpaceTextView *)shareInstance{
    ...
    dispatch_once(&onceToken, ^{
        csiiSpaceTextView = [[JRCSIISpaceTextView alloc] initWithFrame:CGRectMake((ScreenWidth - 645 * scaleW)/2, (ScreenHeight - 745 *scaleH)/2, 645 * scaleW, 845 * scaleH)];
        [((UIWindow*)[[[UIApplication sharedApplication] windows] objectAtIndex:0]).rootViewController.view addSubview:csiiSpaceTextView];
    });
    
    return csiiSpaceTextView;
}
...
- (void)test1 {
    NSString*filePath = [[NSBundle mainBundle]pathForResource:@"Spacetextandtext"ofType:@"json"];
...
-(void)show:(JRCSIISpaceTextViewBlock)JRCSIISpaceTextViewBlock{
    ...
    self.JRCSIISpaceTextViewBlock = JRCSIISpaceTextViewBlock;
    retDicView = [[UIView alloc] initWithFrame:CGRectMake(15, 35+50*scaleH, csiiSpaceTextView.size.width-15, 350*scaleH)];
    [self addSubview:retDicView];
    
    for (int i=0; i<type; i++) {
        leftLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 550*scalePH/type*i, csiiSpaceTextView.size.width/type, 350*scalePH/type)];
        leftLabel.text = [[uiData objectAtIndex:i] valueForKey:@"LeftText"];
        [retDicView addSubview:leftLabel];
        rightLabel = [[UILabel alloc] initWithFrame:CGRectMake(5+csiiSpaceTextView.size.width/type , 550*scalePH/type*i, csiiSpaceTextView.size.width, 350*scalePH/type)];
    ...
    popAnimation.timingFunctions = @[[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut],
                                     ...
                                     [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut]];
    [csiiSpaceTextView.layer addAnimation:popAnimation forKey:nil];
    csiiSpaceTextView.hidden = NO;
}
-(void)craetUI{
    [self creatCancelBtn];
...
//    self.formDic = formData;
    //TODO: 这里执行block
    if (self.JRCSIISpaceTextViewBlock) {
        self.JRCSIISpaceTextViewBlock();
    }
