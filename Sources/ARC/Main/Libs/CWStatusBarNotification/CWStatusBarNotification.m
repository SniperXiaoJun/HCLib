...
@interface CWStatusBarNotification() <UIAlertViewDelegate,UIActionSheetDelegate>
...
//                NSString *originStr = [[NSUserDefaults standardUserDefaults] objectForKey:@"originUrl"];
//                [[NSUserDefaults standardUserDefaults] setObject:originStr forKey:@"navUrl"];
//                [[NSNotificationCenter defaultCenter] postNotificationName:@"startApp" object:nil];
            
            UIActionSheet *actionSheet = [[UIActionSheet alloc]
                                          initWithTitle:@"是否退出调试模式"
                                          delegate:weakSelf
                                          cancelButtonTitle:@"取消"
                                          destructiveButtonTitle:@"确定"
                                          otherButtonTitles:@"刷新",nil];
            
            [actionSheet showInView:self.curViewController.view];
//            actionSheet.actionSheetStyle = UIActionSheetStyleBlackOpaque;
            
//                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"是否退出调试模式" delegate:weakSelf cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
//                
//                [alert show];
            
//                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"确定退出调试模式？" preferredStyle:UIAlertControllerStyleAlert];
//                
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex{
    ...
    if(buttonIndex == 0){ // 确定
        DebugLog(@"0");
        NSString *originStr = [[NSUserDefaults standardUserDefaults] objectForKey:@"originUrl"];
        DebugLog(@"originStr===%@",originStr);
        [[NSUserDefaults standardUserDefaults] setObject:originStr forKey:@"navUrl"];
          [Singleton.notification dismissNotification];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"startApp" object:nil];
    }else if (buttonIndex == 1){ // 刷新
       DebugLog(@"1");
        [[NSNotificationCenter defaultCenter] postNotificationName:@"startApp" object:nil];
    }else{// 取消
         DebugLog(@"其他");  // 取消
