...
- (void)viewDidAppear:(BOOL)animated{
    ...
    DebugLog(@"Rx ----- viewDidAppear");
    
}
- (void)viewWillDisappear:(BOOL)animated{
    [super viewWillDisappear:animated];
    DebugLog(@"Rx----viewWillDisappear");
}
-(void)viewDidDisappear:(BOOL)animated{
    [super viewDidDisappear:animated];
    self.webView.delegate = nil;
    DebugLog(@"Rx----viewDidDisappear");
...
//        NSLog(@"about blank!! return");
    ...
    if ([request.URL.absoluteString rangeOfString:@"zhima_out_to_home"].length != 0) {
        [Singleton.rootViewController popViewControllerAnimated:YES];
    }
...
-(void)closeItemClicked{
    DebugLog(@"popViewControllerAnimated----self.viewControllers---\n%@",self.navigationController.viewControllers);
//    if ([self.navigationController.viewControllers.lastObject isKindOfClass:[JRPublisherH5Vc class]]) {
//        
