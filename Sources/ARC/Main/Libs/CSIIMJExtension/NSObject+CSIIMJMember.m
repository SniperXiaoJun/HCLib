...
@implementation NSObject (CSIIMJMember)
...
- (void)enumerateIvarsWithBlock:(CSIIMJIvarsBlock)block
{
    [self enumerateClassesWithBlock:^(__unsafe_unretained Class c, BOOL *stop) {
        ...
        for (int i = 0; i<outCount; i++) {
            CSIIMJIvar *ivar = [[CSIIMJIvar alloc] initWithIvar:ivars[i] srcObject:self];
            ivar.srcClass = c;
            block(ivar, stop);
        }
...
- (void)enumerateMethodsWithBlock:(CSIIMJMethodsBlock)block
{
    [self enumerateClassesWithBlock:^(__unsafe_unretained Class c, BOOL *stop) {
        ...
        // 2.遍历每一个成员变量
        for (int i = 0; i<outCount; i++) {
            CSIIMJMethod *method = [[CSIIMJMethod alloc] initWithMethod:methods[i] srcObject:self];
            method.srcClass = c;
            block(method, stop);
        }
