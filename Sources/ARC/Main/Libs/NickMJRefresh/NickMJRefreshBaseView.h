...
@class NickMJRefreshBaseView;

#pragma mark - 控件的刷新状态
typedef enum {
	NickMJRefreshStatePulling = 1, // 松开就可以进行刷新的状态
	NickMJRefreshStateNormal = 2, // 普通状态
	NickMJRefreshStateRefreshing = 3, // 正在刷新中的状态
    NickMJRefreshStateWillRefreshing = 4
} NickMJRefreshState;

#pragma mark - 控件的类型
typedef enum {
    NickMJRefreshViewTypeHeader = -1, // 头部控件
    NickMJRefreshViewTypeFooter = 1 // 尾部控件
} NickMJRefreshViewType;

/**
 类的声明
 */
@interface NickMJRefreshBaseView : UIView
#pragma mark - 父控件
...
@property (assign, nonatomic) NickMJRefreshState state;
