...
@interface NickMJRefreshHeaderView()
// 最后的更新时间
@property (nonatomic, strong) NSDate *lastUpdateTime;
@property (nonatomic, weak) UILabel *lastUpdateTimeLabel;
@end

@implementation NickMJRefreshHeaderView
#pragma mark - 控件初始化
/**
 *  时间标签
 */
- (UILabel *)lastUpdateTimeLabel
{
    if (!_lastUpdateTimeLabel) {
        ...
        lastUpdateTimeLabel.textColor = NickMJRefreshLabelTextColor;
        ...
        if(self.dateKey){
            ...
            self.lastUpdateTime = [[NSUserDefaults standardUserDefaults] objectForKey:NickMJRefreshHeaderTimeKey];
        }
    }
...
+ (instancetype)header
{
    return [[NickMJRefreshHeaderView alloc] init];
}

- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        self.pullToRefreshText = NickMJRefreshHeaderPullToRefresh;
        self.releaseToRefreshText = NickMJRefreshHeaderReleaseToRefresh;
        self.refreshingText = NickMJRefreshHeaderRefreshing;
    }
    return self;
}
...
- (void)setLastUpdateTime:(NSDate *)lastUpdateTime
{
    ...
    if(self.dateKey){
        ...
        [[NSUserDefaults standardUserDefaults] setObject:lastUpdateTime forKey:NickMJRefreshHeaderTimeKey];
    }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([NickMJRefreshContentOffset isEqualToString:keyPath]) {
...
- (void)setState:(NickMJRefreshState)state
{
    ...
    // 4.根据状态执行不同的操作
	switch (state) {
		case NickMJRefreshStateNormal: // 下拉可以刷新
        {
            // 刷新完毕
...
#warning 这句代码修复了，top值不断累加的bug
		...
		case NickMJRefreshStatePulling: // 松开可立即刷新
        {
            // 执行动画
            [UIView animateWithDuration:NickMJRefreshFastAnimationDuration animations:^{
		...
		case NickMJRefreshStateRefreshing: // 正在刷新中
        {
            // 执行动画
            [UIView animateWithDuration:NickMJRefreshFastAnimationDuration animations:^{
                // 1.增加滚动区域
