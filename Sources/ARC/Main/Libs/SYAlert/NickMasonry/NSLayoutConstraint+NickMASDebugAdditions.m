...
@implementation NSLayoutConstraint (NickMASDebugAdditions)
...
#if TARGET_OS_IPHONE || TARGET_OS_TV
        descriptionMap = @{
            @(NickMASLayoutPriorityDefaultHigh)      : @"high",
            @(NickMASLayoutPriorityDefaultLow)       : @"low",
            @(NickMASLayoutPriorityDefaultMedium)    : @"medium",
            @(NickMASLayoutPriorityRequired)         : @"required",
            @(NickMASLayoutPriorityFittingSizeLevel) : @"fitting size",
        };
#elif TARGET_OS_MAC
        descriptionMap = @{
            @(NickMASLayoutPriorityDefaultHigh)                 : @"high",
            @(NickMASLayoutPriorityDragThatCanResizeWindow)     : @"drag can resize window",
            @(NickMASLayoutPriorityDefaultMedium)               : @"medium",
            @(NickMASLayoutPriorityWindowSizeStayPut)           : @"window size stay put",
            @(NickMASLayoutPriorityDragThatCannotResizeWindow)  : @"drag cannot resize window",
            @(NickMASLayoutPriorityDefaultLow)                  : @"low",
            @(NickMASLayoutPriorityFittingSizeCompression)      : @"fitting size",
            @(NickMASLayoutPriorityRequired)                    : @"required",
        };
#endif
    });
...
- (NSString *)description {
    ...
    if (self.priority != NickMASLayoutPriorityRequired) {
