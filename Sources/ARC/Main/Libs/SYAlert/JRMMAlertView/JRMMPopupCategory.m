...
@implementation UIColor (JRMMPopup)
...
@implementation UIImage (JRMMPopup)
...
@implementation UIButton (JRMMPopup)
...
@implementation NSString (JRMMPopup)
...
@interface UIView (JRMMPopupInner)

@property (nonatomic, assign, readwrite) NSInteger mm_dimReferenceCount;

@end

@implementation UIView (JRMMPopupInner)
...
@implementation UIView (JRMMPopup)
...
- (UIView *)mm_dimBackgroundView
{
    ...
    if ( !dimView )
    {
        ...
        [dimView mas_makeConstraints:^(NickMASConstraintMaker *make) {
            make.edges.equalTo(self);
        }];
        dimView.alpha = 0.0f;
        dimView.backgroundColor = JRMMHexColor(0x0000007F);
...
- (void)setMm_dimBackgroundBlurEnabled:(BOOL)mm_dimBackgroundBlurEnabled
{
    ...
    {
        self.mm_dimBackgroundView.backgroundColor = JRMMHexColor(0x00000000);
        self.mm_dimBackgroundBlurEffectStyle = self.mm_dimBackgroundBlurEffectStyle;
        self.mm_dimBackgroundBlurView.hidden = NO;
    }
    else
    {
        self.mm_dimBackgroundView.backgroundColor = JRMMHexColor(0x0000007F);
...
- (void)setMm_dimBackgroundBlurEffectStyle:(UIBlurEffectStyle)mm_dimBackgroundBlurEffectStyle
{
    ...
    if ( self.mm_dimBackgroundBlurEnabled )
    {
        ...
        [blurView mas_makeConstraints:^(NickMASConstraintMaker *make) {
...
- (UIView *)mm_dimBackgroundBlurView
{
    ...
    if ( !blurView )
    {
        ...
        if ( [UIVisualEffectView class] )
        {
            ...
            [effectView mas_makeConstraints:^(NickMASConstraintMaker *make) {
                make.edges.equalTo(blurView);
            }];
        }
        else
        {
            blurView.backgroundColor = @[JRMMHexColor(0x000007F),JRMMHexColor(0xFFFFFF7F),JRMMHexColor(0xFFFFFF7F)][self.mm_dimBackgroundBlurEffectStyle];
        }
...
- (void)mm_showDimBackground
{
    ...
    if ( self == [JRMMPopupWindow sharedWindow].attachView )
    {
        [JRMMPopupWindow sharedWindow].hidden = NO;
        [[JRMMPopupWindow sharedWindow] makeKeyAndVisible];
    }
    ...
    [UIView animateWithDuration:self.mm_dimAnimationDuration
                     ...
                     animations:^{
                         ...
                         if ( finished )
                         {
                             ...
                             if ( self == [JRMMPopupWindow sharedWindow].attachView )
                             {
                                 [JRMMPopupWindow sharedWindow].hidden = YES;
                                 [[[UIApplication sharedApplication].delegate window] makeKeyWindow];
                             }
                             else if ( self == [JRMMPopupWindow sharedWindow] )
                             {
                                 self.hidden = YES;
                                 [[[UIApplication sharedApplication].delegate window] makeKeyWindow];
                             }
...
- (void) mm_distributeSpacingHorizontallyWith:(NSArray*)views
{
    ...
    [v0 mas_makeConstraints:^(NickMASConstraintMaker *make) {
    ...
    for ( int i = 0 ; i < views.count; ++i )
    {
        ...
        [obj mas_makeConstraints:^(NickMASConstraintMaker *make) {
            make.left.equalTo(lastSpace.mas_right);
        }];
        
        [space mas_makeConstraints:^(NickMASConstraintMaker *make) {
    ...
    [lastSpace mas_makeConstraints:^(NickMASConstraintMaker *make) {
