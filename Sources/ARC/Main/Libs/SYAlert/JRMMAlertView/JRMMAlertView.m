...
@property (nonatomic, copy) JRMMPopupInputHandler inputHandler;

@end

@implementation JRMMAlertView

- (instancetype) initWithInputTitle:(NSString *)title
                             detail:(NSString *)detail
                        placeholder:(NSString *)inputPlaceholder
                            handler:(JRMMPopupInputHandler)inputHandler
{
    JRMMAlertViewConfig *config = [JRMMAlertViewConfig globalConfig];
    
    NSArray *items =@[
                      JRMMItemMake(config.defaultTextCancel, JRMMItemTypeHighlight, nil),
                      JRMMItemMake(config.defaultTextConfirm, JRMMItemTypeHighlight, nil)
                      ];
                               ...
                               detail:(NSString*)detail
{
    ...
    NSArray *items =@[
                      JRMMItemMake(config.defaultTextOK, JRMMItemTypeHighlight, nil)
                      ];
...
- (instancetype)initWithTitle:(NSString *)title
             ...
             inputPlaceholder:(NSString *)inputPlaceholder
                 inputHandler:(JRMMPopupInputHandler)inputHandler
{
    self = [super init];
    ...
    if ( self )
    {
        ...
        self.layer.borderWidth = JRMM_SPLIT_WIDTH;
        ...
        if ( detail.length > 0 )
        {
            ...
            [self.detailLabel mas_makeConstraints:^(NickMASConstraintMaker *make) {
        ...
        if ( self.inputHandler )
        {
            ...
            [self.inputView mas_makeConstraints:^(NickMASConstraintMaker *make) {
                make.top.equalTo(lastAttribute).offset(10);
                make.left.right.equalTo(self).insets(UIEdgeInsetsMake(0, config.innerMargin, 0, config.innerMargin));
                make.height.mas_equalTo(40);
            }];
            self.inputView.backgroundColor = self.backgroundColor;
            self.inputView.layer.borderWidth = JRMM_SPLIT_WIDTH;
        ...
        for ( NSInteger i = 0 ; i < items.count; ++i )
        {
            ...
            [btn mas_makeConstraints:^(NickMASConstraintMaker *make) {
                ...
                if ( items.count <= 2 )
                {
                    ...
                    {
                        firstButton = btn;
                        make.left.equalTo(self.buttonView.mas_left).offset(-JRMM_SPLIT_WIDTH);
                    }
                    else
                    {
                        make.left.equalTo(lastButton.mas_right).offset(-JRMM_SPLIT_WIDTH);
...
//            btn.titleLabel.font = (item==items.lastObject)?[UIFont boldSystemFontOfSize:config.buttonFontSize]:[UIFont systemFontOfSize:config.buttonFontSize];
            ...
            btn.titleLabel.font = [UIFont boldSystemFontOfSize:config.buttonFontSize];
        }
        [lastButton mas_updateConstraints:^(NickMASConstraintMaker *make) {
            
            if ( items.count <= 2 )
            {
                make.right.equalTo(self.buttonView.mas_right).offset(JRMM_SPLIT_WIDTH);
            }
            else
            {
...
@interface JRMMAlertViewConfig()

@end

@implementation JRMMAlertViewConfig

+ (JRMMAlertViewConfig *)globalConfig
{
    static JRMMAlertViewConfig *config;
    static dispatch_once_t onceToken;
    
    dispatch_once(&onceToken, ^{
        
        config = [JRMMAlertViewConfig new];
...
- (instancetype)init
{
    ...
    if ( self )
    {
        ...
        self.titleColor         = JRMMHexColor(0x333333FF);
        self.detailColor        = JRMMHexColor(0x333333FF);
...
//        self.itemHighlightColor = [UIColor redColor];
        self.itemHighlightColor = RGB_COLOR(38, 131, 251);
