//
//  NickTZImagePickerController.h
//  NickTZImagePickerController
//
//  Created by 谭真 on 15/12/24.
//  Copyright © 2015年 谭真. All rights reserved.
//  version 1.9.5 - 2017.11.15
//  更多信息，请前往项目的github地址：https://github.com/banchichen/NickTZImagePickerController

/*
 经过测试，比起xib的方式，把NickTZAssetCell改用纯代码的方式来写，滑动帧数明显提高了（约提高10帧左右）
...
#define NickTZ_showStatusBarInitial (![[[NSBundle mainBundle] objectForInfoDictionaryKey:@"UIStatusBarHidden"] boolValue])

@protocol NickTZImagePickerControllerDelegate;
@interface NickTZImagePickerController : UINavigationController
...
@property (nonatomic, strong) NSMutableArray<NickTZAssetModel *> *selectedModels;
...
// You can get original photo with asset, by the method [[NickTZImageManager manager] getOriginalPhotoWithAsset:completion:].
// The UIImage Object in photos default width is 828px, you can set it by photoWidth property.
// 这个照片选择器会自己dismiss，当选择器dismiss的时候，会执行下面的handle
// 你也可以设置autoDismiss属性为NO，选择器就不会自己dismis了
// 如果isSelectOriginalPhoto为YES，表明用户选择了原图
// 你可以通过一个asset获得原图，通过这个方法：[[NickTZImageManager manager] getOriginalPhotoWithAsset:completion:]
...
@property (nonatomic, weak) id<NickTZImagePickerControllerDelegate> pickerDelegate;

@end


@protocol NickTZImagePickerControllerDelegate <NSObject>
@optional
// The picker should dismiss itself; when it dismissed these handle will be called.
// You can also set autoDismiss to NO, then the picker don't dismiss itself.
// If isOriginalPhoto is YES, user picked the original photo.
...
// 你可以通过一个asset获得原图，通过这个方法：[[NickTZImageManager manager] getOriginalPhotoWithAsset:completion:]
// photos数组里的UIImage对象，默认是828像素宽，你可以通过设置photoWidth属性的值来改变它
- (void)imagePickerController:(NickTZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto;
- (void)imagePickerController:(NickTZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto infos:(NSArray<NSDictionary *> *)infos;
//- (void)imagePickerControllerDidCancel:(NickTZImagePickerController *)picker __attribute__((deprecated("Use -tz_imagePickerControllerDidCancel:.")));
...
@interface NickTZAlbumPickerController : UIViewController
@property (nonatomic, assign) NSInteger columnNumber;
- (void)configTableView;
@end


@interface UIImage (NickMyBundle)
+ (UIImage *)imageNamedFromMyBundle:(NSString *)name;
@end


@interface NSString (NickTzExtension)
...
@interface NickTZCommonTools : NSObject
+ (BOOL)tz_isIPhoneX;
