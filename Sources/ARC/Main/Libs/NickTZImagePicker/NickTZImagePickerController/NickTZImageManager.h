...
@class NickTZAlbumModel,NickTZAssetModel;
@protocol NickTZImagePickerControllerDelegate;
@interface NickTZImageManager : NSObject
...
@property (assign, nonatomic) id<NickTZImagePickerControllerDelegate> pickerDelegate;
...
/// 默认4列, NickTZPhotoPickerController中的照片collectionView
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NickTZAlbumModel *model))completion;
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<NickTZAlbumModel *> *models))completion;

/// Get Assets 获得Asset数组
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<NickTZAssetModel *> *models))completion;
- (void)getAssetFromFetchResult:(id)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NickTZAssetModel *model))completion;

/// Get photo 获得照片
- (void)getPostImageWithAlbumModel:(NickTZAlbumModel *)model completion:(void (^)(UIImage *postImage))completion;
...
//@interface NickTZSortDescriptor : NSSortDescriptor
//
//@end
