//
//  NickTZPhotoPreviewController.m
...
@interface NickTZPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> {
...
@implementation NickTZPhotoPreviewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [NickTZImageManager manager].shouldFixOrientation = YES;
    __weak typeof(self) weakSelf = self;
    NickTZImagePickerController *_tzImagePickerVc = (NickTZImagePickerController *)weakSelf.navigationController;
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (NickTZ_showStatusBarInitial && iOS7Later) {
        [UIApplication sharedApplication].statusBarHidden = NO;
    }
    [NickTZImageManager manager].shouldFixOrientation = NO;
}
...
- (void)configCustomNaviBar {
    ...
    [_backButton setImage:[UIImage imageNamedFromMyBundle:@"navi_backjr"] forState:UIControlStateNormal];
...
- (void)configBottomToolBar {
    ...
    NickTZImagePickerController *_tzImagePickerVc = (NickTZImagePickerController *)self.navigationController;
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[NickTZPhotoPreviewCell class] forCellWithReuseIdentifier:@"NickTZPhotoPreviewCell"];
    [_collectionView registerClass:[NickTZVideoPreviewCell class] forCellWithReuseIdentifier:@"NickTZVideoPreviewCell"];
    [_collectionView registerClass:[NickTZGifPreviewCell class] forCellWithReuseIdentifier:@"NickTZGifPreviewCell"];
}
...
- (void)configCropView {
    ...
    if (!_tzImagePickerVc.showSelectBtn && _tzImagePickerVc.allowCrop) {
        ...
        [NickTZImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat statusBarHeight = [NickTZCommonTools tz_statusBarHeight];
    ...
    CGFloat toolBarHeight = [NickTZCommonTools tz_isIPhoneX] ? 44 + (83 - 49) : 44;
...
- (void)select:(UIButton *)selectButton {
    NickTZImagePickerController *_tzImagePickerVc = (NickTZImagePickerController *)self.navigationController;
    NickTZAssetModel *model = _models[_currentIndex];
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            ...
            if (model.type == NickTZAssetModelMediaTypeVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
        ...
        for (NickTZAssetModel *model_item in selectedModels) {
            if ([[[NickTZImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[NickTZImageManager manager] getAssetIdentifier:model_item.asset]]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    NickTZAssetModel *model = selectedModelsTmp[i];
    ...
    [self refreshNaviBarAndBottomBarState];
    if (model.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:selectButton.imageView.layer type:NickTZOscillatoryAnimationToBigger];
    }
    [UIView showOscillatoryAnimationWithLayer:_numberImageView.layer type:NickTZOscillatoryAnimationToSmaller];
}
...
- (void)doneButtonClick {
    ...
    if (_tzImagePickerVc.allowCrop) { // 裁剪状态
        ...
        NickTZPhotoPreviewCell *cell = (NickTZPhotoPreviewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
        UIImage *cropedImage = [NickTZImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [NickTZImageCropManager circularClipImage:cropedImage];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    NickTZImagePickerController *_tzImagePickerVc = (NickTZImagePickerController *)self.navigationController;
    NickTZAssetModel *model = _models[indexPath.row];

    NickTZAssetPreviewCell *cell;
    __weak typeof(self) weakSelf = self;
    if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == NickTZAssetModelMediaTypeVideo) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"NickTZVideoPreviewCell" forIndexPath:indexPath];
    } else if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == NickTZAssetModelMediaTypePhotoGif && _tzImagePickerVc.allowPickingGif) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"NickTZGifPreviewCell" forIndexPath:indexPath];
    } else {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"NickTZPhotoPreviewCell" forIndexPath:indexPath];
        NickTZPhotoPreviewCell *photoPreviewCell = (NickTZPhotoPreviewCell *)cell;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[NickTZPhotoPreviewCell class]]) {
        [(NickTZPhotoPreviewCell *)cell recoverSubviews];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[NickTZPhotoPreviewCell class]]) {
        [(NickTZPhotoPreviewCell *)cell recoverSubviews];
    } else if ([cell isKindOfClass:[NickTZVideoPreviewCell class]]) {
        [(NickTZVideoPreviewCell *)cell pausePlayerAndShowNaviBar];
    }
}
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    // 如果正在预览的是视频，隐藏原图按钮
    if (!_isHideNaviBar) {
        if (model.type == NickTZAssetModelMediaTypeVideo) {
    ...
    if (![[NickTZImageManager manager] isPhotoSelectableWithAsset:model.asset]) {
...
- (void)showPhotoBytes {
    [[NickTZImageManager manager] getPhotosBytesWithArray:@[_models[_currentIndex]] completion:^(NSString *totalBytes) {
