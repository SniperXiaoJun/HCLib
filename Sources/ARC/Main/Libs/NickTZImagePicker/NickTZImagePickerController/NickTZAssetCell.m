...
@implementation NickTZAssetCell

- (void)setModel:(NickTZAssetModel *)model {
    _model = model;
    if (iOS8Later) {
        self.representedAssetIdentifier = [[NickTZImageManager manager] getAssetIdentifier:model.asset];
    }
    int32_t imageRequestID = [[NickTZImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
        ...
        if ([self.representedAssetIdentifier isEqualToString:[[NickTZImageManager manager] getAssetIdentifier:model.asset]]) {
...
- (void)setType:(NickTZAssetCellType)type {
    _type = type;
    if (type == NickTZAssetCellTypePhoto || type == NickTZAssetCellTypeLivePhoto || (type == NickTZAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    if (type == NickTZAssetCellTypeVideo) {
    ...
    } else if (type == NickTZAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:_selectImageView.layer type:NickTZOscillatoryAnimationToBigger];
        // 用户选中了该图片，提前获取一下大图
        [self fetchBigImage];
    } else { // 取消选中，取消大图的获取
...
- (void)fetchBigImage {
    _bigImageRequestID = [[NickTZImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
            [self hideProgressView];
        }
...
- (UIImageView *)videoImgView {
    if (_videoImgView == nil) {
        ...
        [videoImgView setImage:[UIImage imageNamedFromMyBundle:@"VideoSendIconjr"]];
...
- (NickTZProgressView *)progressView {
    if (_progressView == nil) {
        _progressView = [[NickTZProgressView alloc] init];
...
@interface NickTZAlbumCell ()
@property (weak, nonatomic) UIImageView *posterImageView;
@property (weak, nonatomic) UILabel *titleLabel;
@end

@implementation NickTZAlbumCell
...
- (void)setModel:(NickTZAlbumModel *)model {
    _model = model;
    ...
    [[NickTZImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
@implementation NickTZAssetCameraCell
