...
@implementation NickTZAssetPreviewCell
...
@implementation NickTZPhotoPreviewCell

- (void)configSubviews {
    self.previewView = [[NickTZPhotoPreviewView alloc] initWithFrame:CGRectZero];
...
@interface NickTZPhotoPreviewView ()<UIScrollViewDelegate>

@end

@implementation NickTZPhotoPreviewView
...
- (void)setModel:(NickTZAssetModel *)model {
    ...
    if (model.type == NickTZAssetModelMediaTypePhotoGif) {
        // 先显示缩略图
        [[NickTZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            self.imageView.image = photo;
            [self resizeSubviews];
            // 再显示gif动图
            [[NickTZImageManager manager] getOriginalPhotoDataWithAsset:model.asset completion:^(NSData *data, NSDictionary *info, BOOL isDegraded) {
                if (!isDegraded) {
...
- (void)setAsset:(id)asset {
    ...
    _asset = asset;
    self.imageRequestID = [[NickTZImageManager manager] getPhotoWithAsset:asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
@implementation NickTZVideoPreviewCell
...
- (void)configMoviePlayer {
    ...
    [[NickTZImageManager manager] getPhotoWithAsset:self.model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        _cover = photo;
    }];
    [[NickTZImageManager manager] getVideoWithAsset:self.model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
@implementation NickTZGifPreviewCell
...
- (void)configPreviewView {
    _previewView = [[NickTZPhotoPreviewView alloc] initWithFrame:CGRectZero];
    __weak typeof(self) weakSelf = self;
