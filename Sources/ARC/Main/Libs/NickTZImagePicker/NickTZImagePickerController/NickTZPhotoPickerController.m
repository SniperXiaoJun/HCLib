...
@interface NickTZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
...
@property (nonatomic, strong) NickTZCollectionView *collectionView;
...
@implementation NickTZPhotoPickerController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    NickTZImagePickerController *tzImagePickerVc = (NickTZImagePickerController *)self.navigationController;
...
- (void)fetchAssetModels {
    ...
    dispatch_sync(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && _isFirstAppear && iOS8Later) {
            [[NickTZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NickTZAlbumModel *model) {
                _model = model;
            ...
            if (_showTakePhotoBtn || !iOS8Later || _isFirstAppear) {
                [[NickTZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<NickTZAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    _collectionView = [[NickTZCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:_layout];
    ...
    [_collectionView registerClass:[NickTZAssetCell class] forCellWithReuseIdentifier:@"NickTZAssetCell"];
    [_collectionView registerClass:[NickTZAssetCameraCell class] forCellWithReuseIdentifier:@"NickTZAssetCameraCell"];
}
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = [NickTZCommonTools tz_isIPhoneX] ? 50 + (83 - 49) : 50;
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (iOS7Later && !isStatusBarHidden) top += [NickTZCommonTools tz_statusBarHeight];
    ...
    if (!self.navigationController.navigationBar.isHidden) {
        ...
        if (iOS7Later) navigationHeight += [NickTZCommonTools tz_statusBarHeight];
    ...
    [NickTZImageManager manager].columnNumber = [NickTZImageManager manager].columnNumber;
...
- (void)previewButtonClick {
    NickTZPhotoPreviewController *photoPreviewVc = [[NickTZPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    [NickTZImageManager manager].shouldFixOrientation = YES;
    __block id alertView;
    for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
        NickTZAssetModel *model = tzImagePickerVc.selectedModels[i];
        [[NickTZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.row >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.row == 0)) && _showTakePhotoBtn) {
        NickTZAssetCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"NickTZAssetCameraCell" forIndexPath:indexPath];
        cell.imageView.image = [UIImage imageNamedFromMyBundle:tzImagePickerVc.takePictureImageName];
        return cell;
    }
    // the cell dipaly photo or video / 展示照片或视频的cell
    NickTZAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"NickTZAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        NickTZImagePickerController *tzImagePickerVc = (NickTZImagePickerController *)weakSelf.navigationController;
        // 1. cancel select / 取消选择
        if (isSelected) {
            weakCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (NickTZAssetModel *model_item in selectedModels) {
                if ([[[NickTZImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[NickTZImageManager manager] getAssetIdentifier:model_item.asset]]) {
        ...
        [UIView showOscillatoryAnimationWithLayer:weakLayer type:NickTZOscillatoryAnimationToSmaller];
    };
    return cell;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NickTZAssetModel *model = _models[index];
    if (model.type == NickTZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            NickTZImagePickerController *imagePickerVc = (NickTZImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both video and photo"]];
        } else {
            NickTZVideoPlayerController *videoPlayerVc = [[NickTZVideoPlayerController alloc] init];
            videoPlayerVc.model = model;
            [self.navigationController pushViewController:videoPlayerVc animated:YES];
        }
    } else if (model.type == NickTZAssetModelMediaTypePhotoGif && tzImagePickerVc.allowPickingGif && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            NickTZImagePickerController *imagePickerVc = (NickTZImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both photo and GIF"]];
        } else {
            NickTZGifPhotoPreviewController *gifPreviewVc = [[NickTZGifPhotoPreviewController alloc] init];
...
- (void)pushImagePickerController {
    // 提前定位
    __weak typeof(self) weakSelf = self;
    [[NickTZLocationManager manager] startLocationWithSuccessBlock:^(CLLocation *location, CLLocation *oldLocation) {
...
- (void)pushPhotoPrevireViewController:(NickTZPhotoPreviewController *)photoPreviewVc {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    NickTZImagePickerController *imagePickerVc = (NickTZImagePickerController *)self.navigationController;
    [[NickTZImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    for (NickTZAssetModel *model in _models) {
        ...
        NickTZImagePickerController *tzImagePickerVc = (NickTZImagePickerController *)self.navigationController;
        for (NickTZAssetModel *model in tzImagePickerVc.selectedModels) {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[NickTZImageManager manager] savePhotoWithImage:photo location:self.location completion:^(NSError *error){
                if (!error) {
...
- (void)resetCachedAssets {
    [[NickTZImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[NickTZImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[NickTZImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            NickTZAssetModel *model = _models[indexPath.item];
...
@implementation NickTZCollectionView
