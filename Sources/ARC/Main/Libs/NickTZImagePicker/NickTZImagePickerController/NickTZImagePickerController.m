...
@interface NickTZImagePickerController () {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
/// Default is 4, Use in photos collectionView in NickTZPhotoPickerController
/// 默认4列, TZPhotoPickerController中的照片collectionView
@property (nonatomic, assign) NSInteger columnNumber;
@end

@implementation NickTZImagePickerController
...
- (void)viewDidLoad {
    ...
    if (iOS7Later) {
        ...
        if (NickTZ_showStatusBarInitial) [UIApplication sharedApplication].statusBarHidden = NO;
    }
}
...
- (void)configBarButtonItemAppearance {
    ...
    if (iOS9Later) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[NickTZImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[NickTZImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<NickTZImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<NickTZImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<NickTZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    _pushPhotoPickerVc = pushPhotoPickerVc;
    NickTZAlbumPickerController *albumPickerVc = [[NickTZAlbumPickerController alloc] init];
    ...
    if (self) {
        ...
        if (![[NickTZImageManager manager] authorizationStatusAuthorized]) {
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    NickTZPhotoPreviewController *previewVc = [[NickTZPhotoPreviewController alloc] init];
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        NickTZPhotoPickerController *photoPickerVc = [[NickTZPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[NickTZImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage completion:^(NickTZAlbumModel *model) {
    ...
    NickTZAlbumPickerController *albumPickerVc = (NickTZAlbumPickerController *)self.visibleViewController;
    if ([albumPickerVc isKindOfClass:[NickTZAlbumPickerController class]]) {
...
- (void)setPickerDelegate:(id<NickTZImagePickerControllerDelegate>)pickerDelegate {
    _pickerDelegate = pickerDelegate;
    [NickTZImageManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [NickTZImageManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}

- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [NickTZImageManager manager].photoWidth = photoWidth;
}
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (id asset in selectedAssets) {
        NickTZAssetModel *model = [NickTZAssetModel modelWithAsset:asset type:[[NickTZImageManager manager] getAssetType:asset]];
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [NickTZImageManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
...
- (void)willInterfaceOrientionChange {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.02 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (![UIApplication sharedApplication].statusBarHidden) {
            if (iOS7Later && NickTZ_showStatusBarInitial) [UIApplication sharedApplication].statusBarHidden = NO;
        }
    });
}
...
@interface NickTZAlbumPickerController ()<UITableViewDataSource,UITableViewDelegate> {
...
@implementation NickTZAlbumPickerController
...
- (void)configTableView {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NickTZImagePickerController *imagePickerVc = (NickTZImagePickerController *)self.navigationController;
        [[NickTZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage completion:^(NSArray<NickTZAlbumModel *> *models) {
            _albumArr = [NSMutableArray arrayWithArray:models];
            for (NickTZAlbumModel *albumModel in _albumArr) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (!_tableView) {
                    ...
                    [_tableView registerClass:[NickTZAlbumCell class] forCellReuseIdentifier:@"NickTZAlbumCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    NickTZAlbumCell *cell = [tableView dequeueReusableCellWithIdentifier:@"NickTZAlbumCell"];
    NickTZImagePickerController *imagePickerVc = (NickTZImagePickerController *)self.navigationController;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NickTZPhotoPickerController *photoPickerVc = [[NickTZPhotoPickerController alloc] init];
    photoPickerVc.columnNumber = self.columnNumber;
    NickTZAlbumModel *model = _albumArr[indexPath.row];
    photoPickerVc.model = model;
...
@implementation NickTZCommonTools
